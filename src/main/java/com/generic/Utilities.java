...
public class Utilities {
	...
	public void clickMenuOption(boolean bClickOnContinue, String sMainMenu, String sSubMenu) {
	    int iClickableCount = 0;
	    boolean bClickable = false;
	    boolean bResult = false;

	    // XPaths converted to Playwright Locators
	    Locator objMainMenuOption = page.locator(
	        "//ul[@id='CoreMenu']//li//span[contains(text(), '" + sMainMenu + "')]"
	    );
	    Locator objSubMenuOption = page.locator(
	        "//ul[@id='CoreMenu']//li//a[contains(text(), '" + sSubMenu + "')]"
	    );
	    ...
	    String bodyClass = bodyLocator.getAttribute("class");
	    if (bodyClass != null && bodyClass.contains("collapse")) {
	        this.clickMenuNavigation();
	    }

	    // ----- MAIN MENU -----
	    if (sMainMenu != null && !sMainMenu.isEmpty()) {
	        Locator objMenuState = page.locator(
	            "//ul[@id='CoreMenu']//span[contains(text(), '" + sMainMenu + "')]/ancestor::li"
	        );
	        String sMenuState = objMenuState.getAttribute("class");

	        if (sMenuState == null || sMenuState.isEmpty()) {
	            this.logReporter(
	                "Failed to click on Main menu option '" + sMainMenu + "' : clickMenuOption()",
	                false,
	                false
	            );
	        } else if (!sMenuState.contains("active")) {
	            // Retry click up to 5 times, similar to Selenium loop
	            iClickableCount = 0;
	            do {
	                try {
	                    objMainMenuOption.waitFor(new Locator.WaitForOptions()
	                            .setState(WaitForSelectorState.VISIBLE));
	                    objMainMenuOption.click();
	                    bClickable = true;
	                    break;
	                } catch (PlaywrightException e) {
	                    // swallow and retry
	                ...
	                iClickableCount++;
	            } while (iClickableCount < 5);

	            this.logReporter("Click on main menu option '" + sMainMenu + "'.", bClickable, false);
	        } else {
	            this.logReporter(
	                "Main menu option '" + sMainMenu + "' already open : clickMenuOption()",
	                true,
	                false
	            );
	        }
	    ...
	    // ----- SUB MENU -----
	    if (sSubMenu != null && !sSubMenu.isEmpty()) {
	        // Emulate getWebElementList using locator.count() + nth()
	        int subMenuCount = objSubMenuOption.count();
	        for (int i = 0; i < subMenuCount; i++) {
	            Locator subMenu = objSubMenuOption.nth(i);
	            String text = subMenu.innerText();
	            System.out.println(text);

	            if (text != null && text.trim().equalsIgnoreCase(sSubMenu)) {
	                iClickableCount = 0;
	                do { // retry up to 10 times, like your Selenium code
	                    try {
	                        subMenu.waitFor(new Locator.WaitForOptions()
	                                .setState(WaitForSelectorState.VISIBLE));
	                        subMenu.click();
	                        bResult = true;
	                        break;
	                ...
	                break; // once we found the matching submenu, no need to continue
	            }
	        }
	        this.logReporter("Click on Sub menu option '" + sSubMenu + "'.", bResult, false);
	    }
	}
	    public void clickOnContinue() {
	        Locator btnContinue = page.locator("button#btnContinue");

	        if (btnContinue.isVisible()) {
	            boolean bResult;
	            try {
	                // Wait until visible before clicking (closest equivalent to clickable)
	                btnContinue.waitFor(new Locator.WaitForOptions()
	                        .setState(WaitForSelectorState.VISIBLE));
	            ...
	            this.logReporter("Click on continue button.", bResult, false);
	        } else {
	            this.logReporter("Continue button not displayed.", true, false);
	        }
	    }
	    public void clickMenuNavigation() {
	        Locator navigate = page.locator("//a[@id='lnkLeftnavigation']");

	        if (navigate.isVisible()) {
	            boolean bResult;
	            try {
	                // Wait until visible before clicking
	                navigate.waitFor(new Locator.WaitForOptions()
	                        .setState(WaitForSelectorState.VISIBLE));
	            ...
	            this.logReporter(
	                "Click on Home Navigation :clickMenuNavigation()",
	                bResult,
	                false
	            );
	        }
	    ...
	    public String getCurrentObjectID() {
	        try {
	            ...
	            // Same XPath, now as a Playwright Locator
	            Locator objObjectID = page.locator(
	                "//header[@class='main-header']//small[@class='label bg-red ec-number']"
	            );

	            // Read minwait from config (assume ms if not documented; adjust if you know it’s seconds)
	            int minWaitMs = 0;
	            try {
	                String minWaitProp = objConfig.getProperty("minwait");
	                if (minWaitProp != null && !minWaitProp.trim().isEmpty()) {
	                    minWaitMs = Integer.parseInt(minWaitProp.trim());
	                }
	            } catch (Exception e) {
	                // If parsing fails, use a small default
	                minWaitMs = 500;
	            }

	            do {
	                try {
	                    // Wait for element to be visible up to minWait
	                    objObjectID.waitFor(new Locator.WaitForOptions()
	                            .setTimeout(minWaitMs)
	                            .setState(com.microsoft.playwright.options.WaitForSelectorState.VISIBLE));

	                    // If found, read its text
	                    String text = objObjectID.innerText();
	                    if (text != null && !text.isEmpty()) {
	                        sObjectID = text;
	                        break;
	                    }
	                } catch (PlaywrightException e) {
	                    // Element not found / not visible within minWait; just retry
	                }
	    ...
	    public void activateTab(String sTabName, String... sParameters) {
	        // RPHADKE_10042018 - updated xpath

	        String sTabClassVal = "";
	        Locator tabOnPage = null;
	        Locator objTabOnPageForClassVal = null;

	        if (sTabName.equals("Action")) { // sz101222018
	            tabOnPage = page.locator("//ul[@class='nav nav-tabs work_flow ChkClick']//a[text()='" + sTabName + "']");
	            objTabOnPageForClassVal = page.locator(
	                "//ul[@class='nav nav-tabs work_flow ChkClick']//a[text()='" + sTabName + "']/parent::li"
	            );
	        } else if (sTabName.equals("Checklists") || sTabName.equals("Workflow")) { // Rupali_080819
	            tabOnPage = page.locator("//ul[contains(@class,'nav nav-tabs')]//a[text()='" + sTabName + "']");
	            objTabOnPageForClassVal = page.locator(
	                "//ul[contains(@class,'nav nav-tabs')]//a[text()='" + sTabName + "']/parent::li"
	            );
	        } else if (sTabName.equals("Order BOM \u2013 Immediate Impact")) { // abraham
	            tabOnPage = page.locator("//*[@id='RevisedProjects']/a[text()='" + sTabName + "']");
	            objTabOnPageForClassVal = page.locator(
	                "//*[@id='RevisedProjects']/a[text()='" + sTabName + "']/parent::li"
	            );
	        } else if (sTabName.equals("ECOPM Group Inbox") || sTabName.equals("ECO PM Group Inbox")) { // abraham
	            tabOnPage = page.locator("//*[@id='ECOPMGroupInbox']/a");
	            objTabOnPageForClassVal = page.locator("//*[@id='ECOPMGroupInbox']/a/parent::li");
	        } else {
	            tabOnPage = page.locator(
	                "//ul[@class='nav nav-tabs work_flow' or 'nav nav-tabs no-border']//a[text()='" + sTabName + "']"
	            );
	            objTabOnPageForClassVal = page.locator(
	                "//ul[@class='nav nav-tabs work_flow' or 'nav nav-tabs no-border']//a[text()='" + sTabName + "']/parent::li"
	            );
	        }

	        boolean clicked = false;

	        if (tabOnPage != null && tabOnPage.isVisible()) {
	            try {
	                // Wait for the tab to be visible (closest to Selenium "clickable")
	                tabOnPage.waitFor(new Locator.WaitForOptions()
	                        .setState(WaitForSelectorState.VISIBLE));

	                // Approximate JavaScript click (like objWrappers.javaScriptClick)
	                tabOnPage.evaluate("element => element.click()");

	                clicked = true;
	            } catch (Exception e) {
	                // Fallback to normal click if JS click fails for some reason
	                try {
	                    tabOnPage.click();
	                    clicked = true;
	                } catch (Exception ex) {
	            ...
	            this.logReporter("Successfully activated '" + sTabName + "' tab on EC main page.", clicked, false);
	            this.waitFor(10); // required to load the page
	        } else {
	            this.logReporter(
	                "Failed to activate '" + sTabName + "' tab on EC main page.:- activateTab()",
	                false,
	                false
	            );
	        }
	        ...
	        this.waitFor(10);
	        // Assuming you’ll update waitTillPageLoad to accept a selector string or Playwright locator
	        this.waitTillPageLoad("div.k-loading-image");
	    }
	    public void waitTillPageLoad(String selector) {
	        int iCounter = 0;

	        // Default selector if none passed (same as original By.cssSelector)
	        if (selector == null || selector.trim().isEmpty()) {
	            selector = "div.blockUI.blockMsg.blockPage";
	        }

	        do {
	            // Your existing wait, likely seconds
	            this.waitFor(10);

	            Locator loader = page.locator(selector);

	            // If element is not visible, consider page load complete
	            if (!loader.isVisible()) {
	                iCounter = 50;
	                break;
	            }
	    ...
	    public void selectSearchOption(String sOption, String sText) {
	        if (ExcelOperations.dataType == "NoData") {
	            ExcelOperations.dataType = "";
	            this.logReporter("excel cell value with Y are finished", false, false);
	        } else {

	            // Locators converted from By.xpath
	            Locator quickSearchIcon = page.locator("//div[@id='searchScopeSelect']/img");
	            Locator searchEditBox = page.locator("//input[(@id='txtEC_Number')]");
	            Locator searchClick = page.locator("//i[@class='fa fa-search']");
	            Locator optionElement = page.locator("//div[contains(text(),'" + sOption + "')]");

	            boolean bResult;

	            // --- Wait for & click Quick Search Icon ---
	            try {
	                quickSearchIcon.waitFor(new Locator.WaitForOptions()
	            ...
	            this.logReporter("Click on Search Button", bResult, false);

	            // --- Set Text in Search Box ---
	            try {
	                searchEditBox.waitFor(new Locator.WaitForOptions()
	                        .setState(WaitForSelectorState.VISIBLE));
	                searchEditBox.fill(sText);
	                bResult = true;
	            } catch (Exception e) {
	                bResult = false;
	            }
	            ...
	            this.logReporter(
	                    "Successfully set text '" + sText + "'.:selectSearchOption()",
	                    bResult,
	                    false
	            );

	            // --- Wait for & click Search Icon ---
	            try {
	                searchClick.waitFor(new Locator.WaitForOptions()
	                        .setState(WaitForSelectorState.ATTACHED));
	                searchClick.click();
	                bResult = true;
	            } catch (Exception e) {
	            ...
	            this.logReporter("Click on  search icon.", bResult, false);
	        }
	    }
	    
	    public String getPageHeader() {
	        Locator objPageHeader = page.locator("//div[@id='PageTitle']/label[@id='heading']");
	        return objPageHeader.innerText();
	    }
}